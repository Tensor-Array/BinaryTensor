...
namespace binary_tensor
{
    namespace value
    {
        ...
        void Tensor::TensorContent::reset_grad()
        {
            ...
            this->grad = zeros(this->buf.shape()).get_buffer();
        }
        ...
        void Tensor::TensorContent::calc_grad_thread(Tensor grad, const std::pair<const Tensor, std::vector<Derivation>>* data_ptr)
        {
            ...
            Tensor temp_sum = zeros(child.get_buffer().shape());
        ...
        Tensor add_dim(const std::vector<Tensor>& list)
        {
            ...
            TensorBase other_buf(last_sizes, content);
        ...
        Tensor Tensor::unslice(const std::initializer_list<unsigned int>& shape_arr, const std::initializer_list<Slice>& slice_arr) const
        {
            ...
            Tensor temp_zeros = zeros(temp_shape_arr);
        ...
        Tensor Tensor::slice(const std::initializer_list<Slice>& slice_arr, bool is_derive) const
        {
            ...
            if (is_derive)
            {
                ...
                temp.push_back(std::make_pair(*this, Derivation(zeros(wrapper::initializer_wrapper(derive_shape.begin(), derive_shape.begin() + slice_arr.size())), derive_slice, false, slice_arr)));
            }
            const Slice slice_begin = this->correct_slice(slice_arr.begin()[0]);
        ...
        {
            return dot(*this, ones(this->get_buffer().shape()));
        }
        ...
        {
            return dat.reshape(new_shape.get_buffer().shape(), false);
        }
        ...
        Tensor Tensor::reshape(const std::initializer_list<unsigned int>& list_dim, bool is_derive) const
        {
            ...
            TensorBase other_buf(list_dim, this->get_buffer().data());
        ...
        Tensor derive_index(const Tensor& in_value, const Tensor&, bool, const DataBuffer& dat)
        {
            ...
            Tensor tensor_temp = zeros(temp_dim);
        ...
        Tensor tensor_file_load(const char* dir)
        {
            if (std::FILE* tensor_file = std::fopen(dir, "rb"))
            {
                char type_wrapper;
                size_t temp_read;
                temp_read = std::fread(&type_wrapper, sizeof(char), 1, tensor_file);
                ...
                const std::type_info& dtype = typeid(dtype::uint1_t_x8);
                ...
                TensorBase t_base(wrapper::initializer_wrapper(temp_shape, temp_shape + shape_size), temp_data, devices::DEVICE_CPU_0);
