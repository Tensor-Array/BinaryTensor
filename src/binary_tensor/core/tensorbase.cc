...
namespace binary_tensor
{
	namespace value
	{
        ...
        {
        public:
            TensorBuf(unsigned char, const unsigned int*, const void*, const devices::Device&, const devices::Device&);
            ...
            const dtype::uint1_t_x8* data() const override;
        ...
        private:
            const dtype::uint1_t_x8* const dat;
            const unsigned int* const sizes;
            const unsigned char dim;
            const devices::Device device;
        };

        TensorBase::TensorBase(const std::initializer_list<unsigned int>& shape_vec, const void* dat, const devices::Device& dev_other, const devices::Device& dev_this):
            instance(std::make_unique<TensorBuf>(static_cast<unsigned char>(shape_vec.size()), shape_vec.begin(), dat, dev_other, dev_this))
        {
        }

        TensorBase::TensorBase(const std::vector<unsigned int>& shape_vec, const void* dat, const devices::Device& dev_other, const devices::Device& dev_this):
            TensorBase(wrapper::initializer_wrapper<unsigned int>(shape_vec.begin().operator->(), shape_vec.end().operator->()), dat, dev_other, dev_this)
        {
        }
        ...
        TensorBase::TensorBase(const TensorBase& other, const devices::Device& other_device) :
            TensorBase(other.shape(), other.data(), other.get_device(), other_device)
        {
        }
        ...
        {
            return typeid(dtype::uint1_t_x8);
        }
        ...
        void TensorBase::save(const char* dir) const
        {
            if (static_cast<bool>(this->instance))
            {
                if (std::FILE* tensor_file = std::fopen(dir, "wb"))
                {
                    ...
                    temp_save = std::fwrite(&temp_type, sizeof(char), 1, tensor_file);
		...
		dtype::uint1_t_x8* create_data_1(size_t other_data_size, const void* other_data, const devices::Device& dev_other, const devices::Device& dev_this)
		{
			dtype::uint1_t_x8* temp_sizes = new (dev_this) dtype::uint1_t_x8[other_data_size];
            if (other_data)
        ...
        {
            return sizeof(dtype::uint1_t_x8);
        }

		unsigned long long get_buffer_size0(unsigned char dim, const unsigned int* sizes)
		{
        ...
        TensorBuf::TensorBuf(unsigned char dim, const unsigned int* sizes, const void* dat, const devices::Device& dev_other, const devices::Device& dev_this) :
            dim(dim),
            sizes(create_arr_dim_sizes(dim, sizes)),
			dat(create_data_1(get_buffer_size0(dim, sizes), dat, dev_other, dev_this)),
            device(dev_this)
        {
        }

        TensorBuf::TensorBuf(const TensorBuf& other) :
            TensorBuf(other.dim, other.sizes, other.dat, other.device, other.device)
        {
        }
        ...
        {
            delete[] this->sizes;
            operator delete[](const_cast<dtype::uint1_t_x8*>(this->dat), this->device);
        }
        ...
        const dtype::uint1_t_x8* TensorBuf::data() const
        {
            return this->dat;
        }
        ...
        {
            return get_buffer_size0(this->dim, this->sizes);
        }
        std::unique_ptr<TensorBase::TensorStorage> TensorBuf::child_create(unsigned int index) const
        {
            ...
            return std::make_unique<TensorBuf>(this->dim - 1, &this->sizes[1], data_ptr, this->device, this->device);
        }
